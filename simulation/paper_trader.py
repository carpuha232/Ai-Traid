...
class PaperTrader:
    ...
    def open_position(self, signal: 'TradingSignal', orderbook: Dict, adaptive_params: Dict = None) -> Optional[Position]:
        ...
        # Рассчитываем запас до ликвидации для логирования
        if signal.direction == 'LONG':
            stop_dist_pct = abs(entry_price - signal.stop_loss) / entry_price * 100
            liq_dist_pct = abs(entry_price - liquidation_price) / entry_price * 100
        else:
            stop_dist_pct = abs(signal.stop_loss - entry_price) / entry_price * 100
