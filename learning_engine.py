...
class ProgressiveLearningEngine:
    ...
    def __init__(self, initial_capital: float = 100.0):
        ...
        self.learning_config = {
            'target_win_rate': 0.65,  # Целевой винрейт 65%+
            ...
            'evaluation_cycles': 5,      # Оценка каждые 5 циклов (чаще)
    ...
    def start_progressive_learning(self):
        ...
        print("=" * 60)
        print("🎯 Цель: винрейт 65%+ на каждом таймфрейме")
        ...
        while True:  # Бесконечный цикл
            ...
            while self.current_timeframe_index < len(self.timeframe_progression):
                ...
                if success:
                    print(f"✅ ДОСТИГНУТ ВИНРЕЙТ 65%+ НА {self.current_timeframe}")
    ...
    def _learn_on_timeframe(self) -> bool:
        ...
        print(f"🔄 БЕСКОНЕЧНОЕ ОБУЧЕНИЕ НА {self.current_timeframe} ДО ДОСТИЖЕНИЯ 65%+ ВИНРЕЙТА")
                ...
                if avg_win_rate >= self.learning_config['target_win_rate']:
                    ...
                    if len(all_profits) > 2:
                        ...
                        if total_profit > 0 and top_2_sum / total_profit > 0.9:
                            ...
                            print(f"🎯 ДОСТИГНУТ ЦЕЛЕВОЙ ВИНРЕЙТ {avg_win_rate:.1%} >= 65%! Переход к следующему таймфрейму.")
                            return True
                    else:
                        print(f"🎯 ДОСТИГНУТ ЦЕЛЕВОЙ ВИНРЕЙТ {avg_win_rate:.1%} >= 65%! Переход к следующему таймфрейму.")
                        return True
    ...
    def _execute_timeframe_cycle(self) -> Dict:
        ...
        # 🧠 СБРОС КАПИТАЛА ДО $100 В КАЖДОМ ЦИКЛЕ
        self.neural_network.current_capital = 100.0
        print(f"   💰 Сброс капитала до $100 в начале цикла")
