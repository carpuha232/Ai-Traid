...
class SignalAnalyzer:
    ...
    def analyze(self, symbol: str, orderbook: Dict, recent_trades: List[Dict]) -> TradingSignal:
        ...
        # 🛡️ РИСК-МЕНЕДЖМЕНТ С ЗАЩИТОЙ ОТ ЛИКВИДАЦИИ:
        # Стоп-лосс должен быть МЕЖДУ входом и ликвидацией!
        # Расстояние до ликвидации = 100 / leverage (например 2% при 50x)
        # Стоп-лосс = 70% от расстояния до ликвидации (запас 30%)
        
        leverage = self.config['account'].get('leverage', 50)
        liquidation_distance = 100.0 / leverage  # Например 2% при 50x
        
        # Стоп на 70% пути к ликвидации (например 1.4% при 50x)
        # Это гарантирует что стоп сработает ДО ликвидации
        stop_distance_percent = liquidation_distance * 0.70
        
        # Take profit остается как раньше (можем использовать multiplier из конфига)
        tp_multiplier = self.config['risk'].get('take_profit_multiplier', 2.0)
        take_profit_distance = stop_distance_percent * tp_multiplier
        
        if direction == 'LONG':
            entry_price = best_ask  # Входим по Ask
            # Stop ВЫШЕ ликвидации (между входом и ликвидацией)
            stop_loss = entry_price * (1 - stop_distance_percent / 100)  # Например -1.4% при 50x
            take_profit_1 = entry_price * (1 + take_profit_distance / 100)
            take_profit_2 = entry_price * (1 + take_profit_distance * PI / 100)
        else:  # SHORT
            entry_price = best_bid  # Входим по Bid
            # Stop НИЖЕ ликвидации (между входом и ликвидацией)
            stop_loss = entry_price * (1 + stop_distance_percent / 100)  # Например +1.4% при 50x
            take_profit_1 = entry_price * (1 - take_profit_distance / 100)
            take_profit_2 = entry_price * (1 - take_profit_distance * PI / 100)
        ...
        # Рассчитываем предполагаемую цену ликвидации для логирования
        expected_liquidation = entry_price * (1 - liquidation_distance / 100) if direction == 'LONG' else entry_price * (1 + liquidation_distance / 100)
        ...
        logger.info(f"   📍 Entry: ${entry_price:.4f} | Stop: ${stop_loss:.4f} ({stop_distance_percent:.2f}%) | Liquidation: ~${expected_liquidation:.4f} ({liquidation_distance:.2f}%) | TP1: ${take_profit_1:.4f}")
