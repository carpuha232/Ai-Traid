...
class AutoScalpingBot:
    ...
    def _monitor_position_protection(self):
        """Monitor positions and manage stepped stop-loss protection."""
        if self.mode != 'live_trading':
            return
        ...
        for symbol, position in list(self.paper_trader.positions.items()):
            # Skip if no PNL data
            ...
            # Calculate what the stop level should be
            target_stop_pnl = risk_manager.calculate_stepped_stop_loss(pnl_pct)
            
            if target_stop_pnl is None:
                # PNL < 10%, no protection needed yet
                continue
            
            # Check if we need to place or update the stepped stop
            if not position.stepped_stop_active or position.stepped_stop_level_pnl != target_stop_pnl:
                logger.info(
                    f"🛡️ {symbol}: PNL={pnl_pct:.1f}% → Updating stepped stop to +{target_stop_pnl:.1f}%"
                )
                
                # Place/update stepped stop order
                order_id = risk_manager.place_stepped_stop_order(
                    position=position,
                ...
                if order_id:
                    ...
                    position.stepped_stop_order_id = order_id
                    
                    # Mark position as protected when first stop is placed
                    if not position.is_protected and target_stop_pnl >= 10.0:
                        position.is_protected = True
                        position.protection_activated_at = pnl_pct
                        
                        # Cancel averaging order when position becomes protected
                        if hasattr(position, 'averaging_order_id') and position.averaging_order_id:
                            success = risk_manager.cancel_order(
                                symbol=symbol,
                                order_id=position.averaging_order_id
                            )
                            if success:
                                logger.info(f"🗑️ {symbol}: Averaging order {position.averaging_order_id} canceled (position protected)")
                                position.averaging_order_id = None
                        
                        logger.info(
                            f"✅ {symbol}: Position PROTECTED (stop @ +{target_stop_pnl:.1f}%)"
                        )
    ...
    async def start(self):
        ...
        try:
            ...
            position_refresh_interval = 5  # seconds - faster PNL updates
            ...
            while self.running:
                try:
                    ...
                    if self.mode == 'live_trading' and now - last_position_refresh >= position_refresh_interval:
                        ...
                        # ✅ Monitor positions for stepped stop-loss protection
