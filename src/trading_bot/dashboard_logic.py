...
class DashboardLogic:
    ...
    def _simulate_trading(self, ind, symbols, timeframes, trades_per_individual):
        """
        Симуляция торговли для одного индивида (с правильной механикой расчета).
        Добавлен margin-call: если баланс становится <= 0, все открытые позиции закрываются,
        логируется событие margin-call, симуляция перезапускается с балансом $100.
        Сохраняется счетчик margin-call.
        """
        ...
        while t < trades_per_individual:
            try:
                ...
                entry_price = random.uniform(10, 100)  # Заглушка, заменить на реальную цену
                ...
                # Реалистичная вероятность выигрыша на основе параметров индивида
                base_winrate = 0.5
                if ind.position_size < 0.02:
                    base_winrate += 0.1
                if ind.leverage < 5:
                    base_winrate += 0.05
                if ind.budget_frac < 0.5:
                    base_winrate += 0.05
                winrate = min(0.8, max(0.2, base_winrate + random.uniform(-0.1, 0.1)))
                ...
                old_balance = balance
                balance += final_pnl
                self.add_terminal_message(f"[МЕХАНИКА] Баланс до: ${old_balance:.2f}", "DEBUG")
                self.add_terminal_message(f"[МЕХАНИКА] Риск: ${risk_amount:.2f} ({ind.position_size*100:.1f}% от баланса)", "DEBUG")
                self.add_terminal_message(f"[МЕХАНИКА] Плечо: {leverage}x, Маржа: ${margin_used:.2f}", "DEBUG")
                self.add_terminal_message(f"[МЕХАНИКА] Размер позиции: {position_size:.4f} монет (${position_volume:.2f})", "DEBUG")
                self.add_terminal_message(f"[МЕХАНИКА] PnL: ${pnl:+.2f} ({pnl_pct*100:+.1f}% * {leverage}x = {pnl_pct*leverage*100:+.1f}%)", "DEBUG")
                self.add_terminal_message(f"[МЕХАНИКА] Комиссия: ${commission:.2f} ({commission_rate*200:.2f}% от объема)", "DEBUG")
                self.add_terminal_message(f"[МЕХАНИКА] Финальный PnL: ${final_pnl:+.2f}", "DEBUG")
                self.add_terminal_message(f"[МЕХАНИКА] Баланс после: ${balance:.2f} (изменение: {final_pnl:+.2f})", "DEBUG")
    ...
    def get_sync_status(self) -> str:
        ...
        return self.sync_manager.get_sync_status_message() 